...
def test_autofill_with_preset_resolves_time_fraction_cues(tmp_path) -> None:
    ...
    # so beat-indexed cues (0,8,32,64,128) all resolve; time-fraction cues
    # resolve to nearest beat within 0-149.5s range
    assert pad_slots[0]["label"] == "Intro"
    assert pad_slots[0]["timestamp_seconds"] == 0.0

    # Breakdown at 40% of 200s = 80s → nearest beat at 80.0s (beat index 160)
    breakdown = next(p for p in pad_slots if p["label"] == "Breakdown")
    assert breakdown["timestamp_seconds"] == pytest.approx(80.0, abs=0.6)

    # Outro at 88% of 200s = 176s; last beat is 149.5s so snaps to end
